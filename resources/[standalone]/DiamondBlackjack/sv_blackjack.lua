...
-- PenguRP: bet limits (chips are 1:1 with cash, no separate item needed)
local penguMinBet = 10     -- PenguRP
local penguMaxBet = 10000  -- PenguRP

function tryTakeChips(source,amount)
    --returns true if taken chips succesfully
    --returns false if doesn't have enough chips
    -- PenguRP: wired to Qbox cash, server-authoritative
    amount = math.floor(tonumber(amount) or 0)
    if amount < penguMinBet or amount > penguMaxBet then
        TriggerClientEvent("blackjack:notify",source,"~r~Bets must be between " .. penguMinBet .. " and " .. penguMaxBet .. " chips.")
        return false
    end
    return exports.qbx_core:RemoveMoney(source, 'cash', amount, 'casino-blackjack-bet') == true
end
...
function giveChips(source,amount)
    ...
    exports.qbx_core:AddMoney(source, 'cash', amount, 'casino-blackjack-win')
end
...
AddEventHandler("Blackjack:requestSitAtBlackjackTable", function(chairId)
    ...
    -- PenguRP: validate chair id from client and reject occupied seats
    if type(chairId) ~= "number" or chairId ~= math.floor(chairId) or chairId < 0 or chairId > 127 or blackjackTables[chairId] ~= false then
        return
    end
...
-- PenguRP: guard hit/stand against invalid gameId/nextCardCount sent by clients
local function penguValidHitStand(gameId,src,nextCardCount) -- PenguRP
    if type(nextCardCount) ~= "number" or nextCardCount ~= math.floor(nextCardCount) then return false end
    if blackjackGameData[gameId] == nil then return false end
    if type(blackjackGameData[gameId][src]) ~= "table" then return false end
    if type(blackjackGameData[gameId][src][2]) ~= "table" then return false end
    return true
end
...
AddEventHandler("Blackjack:hitBlackjack",function(gameId,nextCardCount)
    local source = source
    if not penguValidHitStand(gameId,source,nextCardCount) then return end -- PenguRP
