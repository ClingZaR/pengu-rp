...
## PenguRP modifications (2026-07-01)
- Bundled rubbertoe98/RageUI fork src/ per install instructions (RageShared.lua dropped, not referenced by fxmanifest).
- Removed stream/Peds, peds.meta and dlc_vinewood audio data files: server enforces game build 3751 where all casino DLC peds/audio/anims are base-game; re-streaming them can conflict.
- sv_blackjack.lua: tryTakeChips/giveChips wired to Qbox cash via exports.qbx_core RemoveMoney/AddMoney (server-authoritative, logged by qbx_core). Bet limits penguMinBet=10 / penguMaxBet=10000. Added validation guards on sit/hit/stand net events. All edits tagged -- PenguRP.
- No casino_chips item required; chips are 1:1 with cash.
