...
-- PenguRP: rewritten for ox_inventory. The original called qb-inventory exports
-- PenguRP: (getGloveboxes/getTruck) which do not exist on this server.
local function search_vehicle(Type, plate)
    local illegal_items = Config.k9.illegal_items
    local items_list = nil

    if Type == 1 then
        items_list = ox_inventory:GetInventory('glovebox' .. plate) -- PenguRP
    elseif Type == 2 then
        items_list = ox_inventory:GetInventory('trunk' .. plate) -- PenguRP
    end

    if items_list and items_list.items then -- PenguRP: nil-guard (inventory may not be loaded yet)
