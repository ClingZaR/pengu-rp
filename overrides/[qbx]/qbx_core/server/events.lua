...
RegisterNetEvent('QBCore:ToggleDuty', function()
    ...
    if player.PlayerData.job.onduty then
        ...
        -- PenguRP: duty status goes to chat (qbx_chat_theme 'pengu:admin' template), not a toast.
        TriggerClientEvent('chat:addMessage', src, { templateId = 'pengu:admin', args = { 'DUTY', locale('info.off_duty'), 'err' } })
    else
        player.Functions.SetJobDuty(true)
        TriggerClientEvent('chat:addMessage', src, { templateId = 'pengu:admin', args = { 'DUTY', locale('info.on_duty'), 'ok' } })
    end
end)
