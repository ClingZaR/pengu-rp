...
RegisterNetEvent('qbx_properties:server:buyProperty', function(propertyId)
    ...
    -- PenguRP: pay 5% sale commission to the realestate society account; banking failure must never block the sale
    local commission = math.floor(property.price * 0.05)
    if commission > 0 then
        pcall(function()
            exports['Renewed-Banking']:CreateJobAccount({ name = 'realestate', label = 'Real Estate' }, 0)
            exports['Renewed-Banking']:addAccountMoney('realestate', commission)
        end)
    end
