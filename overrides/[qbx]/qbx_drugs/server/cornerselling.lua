...
lib.callback.register('qb-drugs:server:getDrugOffer', function(source)
    ...
    -- PenguRP: scale the rolled price by the server-wide demand factor (model lives in pengu_drugs; 1.0 when unavailable)
    local demandOk, demand = pcall(function() return exports.pengu_drugs:GetDemand(chosenDrug.item) end)
    basePrice = math.max(1, math.floor(basePrice * ((demandOk and tonumber(demand)) or 1.0) + 0.5))
...
RegisterNetEvent('qb-drugs:server:sellCornerDrugs', function(drugType, amount, price)
    ...
    if hasItem.amount >= amount then
        ...
        -- PenguRP: report the completed sale to the server-wide demand model (pengu_drugs)
        pcall(function() exports.pengu_drugs:RecordDrugSale(item, amount) end)
