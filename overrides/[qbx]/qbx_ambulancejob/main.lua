...
-- PenguRP: medikit ($4500 black market doctor) = stronger firstaid. Revives + fully
-- heals a downed player nearby, otherwise fully heals the user. Item check/removal and
-- proximity are server-side; the heal is qbx_medical:client:playerRevived, the same
-- event the firstaid revive flow ends in (full heal + clears all wounds/bleeds).
---@param targetId number?
RegisterNetEvent('hospital:server:UseMedikit', function(targetId)
	...
	local healTarget = src
	if targetId and targetId ~= src and exports.qbx_core:GetPlayer(targetId) then
		local dist = #(GetEntityCoords(GetPlayerPed(src)) - GetEntityCoords(GetPlayerPed(targetId)))
		if dist < 10.0 and lib.callback.await('hospital:client:medikitCanRevive', targetId) then
			healTarget = targetId
		end
	end
...
-- PenguRP: item removal happens in hospital:server:UseMedikit, same split as firstaid
exports.qbx_core:CreateUseableItem('medikit', function(source, item)
	triggerItemEventOnPlayer(source, item, 'hospital:client:UseMedikit')
end)
