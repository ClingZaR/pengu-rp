...
RegisterNetEvent('hospital:client:CheckStatus', function()
    ...
    for hash in pairs(status.damageCauses) do
        TriggerEvent('chat:addMessage', {
            templateId = 'rp:status', -- qbx_chat_theme: render as themed .rp-status line
            multiline = false,
            args = { locale('info.status'), WEAPONS?[hash]?.damagereason or 'Unknown' } -- nil-safe (matches damage.lua); an unregistered damage cause would otherwise crash the death message
        })
    end
