...
function SendReport(source, message)
    ...
    -- PenguRP: mirror /report to Discord when the pengu_report_webhook convar is set
    local webhook = GetConvar('pengu_report_webhook', '')
    if webhook ~= '' then
        local payload = json.encode({
            username = 'PenguRP Reports',
            embeds = {
                {
                    title = ('Report #%d from %s (id %s)'):format(reportsCount, GetPlayerName(source) or 'Unknown', source),
                    description = message,
                    color = 12615679,
                }
            }
