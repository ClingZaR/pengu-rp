...
const Open = (data) => {
  ...
  // PenguRP: per-job on-duty counts (data.dutyCounts = [{label, count}, ...])
  $(".scoreboard-info .duty-beam").remove();
  if (data.dutyCounts && data.dutyCounts.length) {
    let dutyHtml = "";
    $.each(data.dutyCounts, (i, job) => {
      dutyHtml += `
      <div class="scoreboard-info-beam duty-beam">
        <div class="info-beam-title">
            <p>${job.label} On Duty</p>
        </div>
        <div class="info-beam-status" style="color: #ededed"><p>${job.count}</p></div>
      </div>
    `;
    });
    const totalRow = $("#total-players").closest(".scoreboard-info-beam");
    if (totalRow.length) totalRow.before(dutyHtml);
    else $(".scoreboard-info").append(dutyHtml);
  }
};
