return {
    -- Density of various things in the world
    -- Minimum value is 0.0, maximum value is 1.0
    -- Value of 1.0 represents GTA Online populate rates
    -- Value of 0.0 removes all of that type
    -- PenguRP: mild reduction from stock 0.8 for client FPS and sync load headroom
    parked = 0.6, -- Density of parked vehicles
    vehicle = 0.6, -- Density of vehicles
    randomvehicles = 0.6, -- Density of random vehicles
    peds = 0.7, -- Density of random peds (civilians, etc.)
    scenario = 0.7, -- Density of scenario peds (bikers, gangsters, etc.)
}