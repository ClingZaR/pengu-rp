...
ps.registerCallback(resourceName .. ':server:sendToJail', function(source, payload)
    ...
    -- PenguRP: drive the actual jailing through xt-prison. The legacy police:client:SendToJail
    -- event has NO listener while xt-prison is running (qbx_police skips it), so it used to fire
    -- into the void: the citizen got flagged 'injail' in metadata but was never confined. xt-prison's
    -- JailPlayerById export teleports + books the player and sets the 'injail' metadata itself.
    -- `sentence` is the total months from the penal-code charges; xt-prison treats jailTime months
    -- as real minutes (1 = 60s), so it passes straight through with no conversion.
    if GetResourceState('xt-prison'):find('start') then
        exports['xt-prison']:JailPlayerById(targetSource, sentence)
    else
